...
defmodule Codegen.TS do
  ...
  def run(args) do
    ...
    ts = generate(schema)
    # Skip prettier for now to avoid issues
    # ts = ts |> prettier()
  ...
  defp prettier(code) do
    ...
    File.write!(tmp, code)
    # Run prettier from the CLI directory where package.json lives
    cli_dir = Path.expand("../cli", __DIR__)
    {output, status} = System.cmd("npx", ["prettier", "--write", tmp], cd: cli_dir, stderr_to_stdout: true)
    if status != 0 do
      IO.warn("prettier failed: #{output}")
